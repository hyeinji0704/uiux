$(document).ready(function(){
    ...
    let sec05_obj_top
    let sec05_offset
    let scrolling //현재 브라우저가 스크롤된 값
    let window_h //브라우저의 높이 
    let offset = 200 //콘텐츠가 하단에서부터 offset값만큼 올라오면 애니메이션 실행
    ...
    function scroll_chk(){
        ...
        if((sec05_obj_top - scrolling - window_h) < -offset){
            sec05_offset = (sec05_obj_top - scrolling - window_h + offset)*0.6
            $('.sec05 .tit').attr('style', 'transform: translate(0, '+ sec05_offset +'px);')
        }else{
            $('.sec05 .tit').attr('style', 'transform: translate(0, 0);')
        }
    }
    scroll_chk() //문서가 로딩되었을 때
    $(window).resize(function(){
        scroll_chk()//브라우저의 사이즈가 변경되었을 때
    })
